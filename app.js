...
while (cadena != "cancelar"){  //mientras la palabra no sea cancelar ingresa al while
...
while (cadena3 != "cancelar"){ //valido ingreso 
...
for (let i = 0; i < cadena4.length; i++){ //ciclo para recorrer el array
...
function cargacomentarios(){
    ...
    switch (true) { //analizo que dimensión fue seleccionada mediante switch
...
function juego(){ //Función del juego
    if (int < 3){  //Permite jugar siempre que los intentos sean menos de 3
        if (nums != document.getElementById("numintento").value){ //compara el valor ingresado con el num secreto y si
            document.getElementById("resultado").value = "Error, numero incorrecto. Intente nuevamente" //no es el num
        ...
        else{ //en el caso de que sea el numero secreto dentro de los 3 intentos
